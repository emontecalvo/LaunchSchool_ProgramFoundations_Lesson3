...
munsters.each do |munster_name, munster_data|
  munster_data.each do |k, v|
    if v == "male"
      a += munster_data["age"].to_i
    end
  end
  
end

puts "total age for all the male members of the family is: #{a}."
...
# answer:
munsters.each do |munster_name, munster_data|
  puts "#{munster_name} is a #{munster_data["age"]} year-old #{munster_data["gender"]}."
end
...
# We learned that when the above "coincidentally" does what we
...
# answer - add a return to the method
def tricky_method(a_string_param, an_array_param)
  a_string_param += "rutabaga"
  an_array_param << "rutabaga"
...
# Our natural inclination is to think of the words in a sentence as 
# a collection. We would like to be able to operate on those words
# the same way we operate on the elements of an array. Where are the 
# String#each_word and the String#map_words methods?

# A common idiom used to solve this conundrum is to use the 
# String#split in conjunction with Array#join methods to break our 
# string up and then put it back together again.
...
a = sentence.split.reverse.join(" ").delete(".") + "."
...
# answer: the output will be 34 instead of 42 because
# the method isn't modifying "answer", it's just using "answer"
# as part of the method.  new_answer is 50 by the end of this.
...
# Did the family's data get ransacked, or did the mischief only 
# mangle a local copy of the original hash? (why?)


# answer: the data did get ransacked
# there isn't a variable with a substitute-hash or re-assignmnet,
# the method really mutates the original hash data
...
#  a function called rps as follows, which follows the classic rules 
# of rock-paper-scissors game, but with a slight twist that it 
...
# answer:  foo will output "yes" regardless so bar output will be "no"
